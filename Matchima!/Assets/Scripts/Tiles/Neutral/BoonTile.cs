...
public class BoonTile : Tile {
	...
	{
		get{
			return new StCon[]{new StCon("Contains a Boon for " + Player.Classes[(int)Genus].Name, GameData.Colour(Genus))};
		}
	}
	...
	//CHANGE ITEM STATS BASED ON VALUE
		...
		Upgrades = new UpgradeGroup(target, target.RollUpgrades(Upgrade_num, Stats.Value));
