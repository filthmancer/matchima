	...
	public override IEnumerator AfterTurn()
	{
		...
		foreach(Class child in Player.Classes)
		{
			...
			if(child.ManaThisTurn > 0)
			{
				...
				{
					//TileMaster.instance.CreateMiniTile(UIManager.CrewButtons[child.Index].transform.position,
					//								UIManager.CrewButtons[Parent.Index].transform, 
					//								TileMaster.Genus.Frame[Parent.Index]);

				//	MoveToPoint mini = TileMaster.instance.CreateMiniTile(	
				//		UIManager.CrewButtons[child.Index].transform.position,
				//		UIManager.CrewButtons[Parent.Index].transform, 
				//		TileMaster.Genus.Frame[Parent.Index]);
		...
		if(total_stolen > 0)
		{
			...
			UIManager.instance.MiniAlert(UIManager.CrewButtons[Parent.Index].transform.position + Vector3.up*0.6F, "Stole " + total_stolen + " Mana!", 40, GameData.Colour(Parent.Genus), 0.8F, 0.1F, true);
