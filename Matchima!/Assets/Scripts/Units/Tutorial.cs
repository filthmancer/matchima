...
public class Tutorial : Wave {
	...
	public override void OnStart(int _index)
	{
		...
		tute.AddQuote("Ten Hut!",  this, true, 1.4F);
		tute.AddQuote("Alright scoundrel, welcome to the Undercity.",  this, true, 1.4F);
		tute.AddQuote("The Council has judged your crime and sentenced you here.",  this, true, 1.4F);
		tute.AddQuote("You must clear the Undercity of this filthy plague of Mana!",  this, true, 1.4F);
		tute.AddQuote("...",  Player.Classes[0], true, 1.4F, 0.35F);
		tute.AddQuote("Well? Don't just stand there! Get cleaning!",  this, true, 1.4F);
		tute.AddQuote("Drag through Mana to collect it!",  this, true, 1.4F);
	...
	public override IEnumerator BeginTurn()
	{
		switch(Player.instance.Turns)
		{
			case 1:
			QuoteGroup tute = new QuoteGroup("Tute");
			tute.AddQuote("Good!",  this, true, 1.4F);
			tute.AddQuote("Mana can be collected in groups of 3 or more!", this, true, 1.4F);
			tute.AddQuote("You can collect tiles in any direction.", this, true, 1.4F);
			...
			tute.AddQuote("Some tiles take multiple hits to collect.",  this, true, 1.4F);
			tute.AddQuote("The number of hits needed will show underneath the tile.",  this, true, 1.4F);
			yield return StartCoroutine(UIManager.instance.Quote(tute.ToArray()));
			break;
			case 5:
			...
			case 8:
			tute = new QuoteGroup("Tute");
			tute.AddQuote("Match enemies to attack them!",  this, true, 1.4F);
			tute.AddQuote("Keep their numbers down!",  this, true, 1.4F);
			yield return StartCoroutine(UIManager.instance.Quote(tute.ToArray()));
			...
			case 9:
			tute = new QuoteGroup("Tute");
			tute.AddQuote("Enemy tiles attack your health at the end of every turn.",  this, true, 1.4F);
			tute.AddQuote("If you lose all your health, you'll all die!",  this, true, 1.4F);
			tute.AddQuote("Luckily, you can regain health by grabbing Health Tiles.",  this, true, 1.4F);
			yield return StartCoroutine(UIManager.instance.Quote(tute.ToArray()));
			break;
			
			case 12:
			tute = new QuoteGroup("Tute");
			tute.AddQuote("Collecting enough mana makes you Level Up!",  this, true, 1.4F);
			tute.AddQuote("Leveling up increases your stats and creates a magic tile.",  this, true, 1.4F);
			yield return StartCoroutine(UIManager.instance.Quote(tute.ToArray()));
			break;
		}
	...
	{
		QuoteGroup tute = new QuoteGroup("Tute");
		tute.AddQuote("Doing fine, prisoner #3.",  this, true, 1.4F);
		tute.AddQuote("But your friends aren't pulling their weight!",  this, true, 1.4F);
		tute.AddQuote("Stop lurking around back there, come do your civic duty!",  this, true, 1.4F);
		yield return StartCoroutine(UIManager.instance.Quote(tute.ToArray()));
		...
		tute.AddQuote("Keep it down vermin!",  this, true, 1.4F);
		tute.AddQuote("Tiles can only be collected with the same colour.",  this, true, 1.4F);
		tute.AddQuote("I want to see you all working hard!",  this, true, 1.4F);
		yield return StartCoroutine(UIManager.instance.Quote(tute.ToArray()));
		yield return null;
	}
	...
	IEnumerator AddBard()
	{
		...
		tute.AddQuote("Save it for your freeloading pals!", this, true, 1.4F);
		tute.AddQuote("You're here to clean up mana and you'd better do it fast!", this, true, 1.4F);
		tute.AddQuote("There's worse things down here than hard labour...", this, true, 1.4F);
		yield return StartCoroutine(UIManager.instance.Quote(tute.ToArray()));
	}
	...
	{
		QuoteGroup tute = new QuoteGroup("Tute");
		tute.AddQuote("Someone wake up the vagrant.", this, true, 1.4F);
		...
		tute.AddQuote("All of you keep it down!", this, true, 1.4F, 0.08F);
		yield return StartCoroutine(UIManager.instance.Quote(tute.ToArray()));
		...
		tute.AddQuote("Oh, nice going prisoners! You've disturbed the wildlife!", this, true, 1.4F);
		yield return StartCoroutine(UIManager.instance.Quote(tute.ToArray()));

		GameManager.instance.GetWave(GameData.instance.GetWave("Red Grunts"), 1);

		tute = new QuoteGroup("Tute");
		tute.AddQuote("Ugh, what's that foul thing?",  Player.Classes[3], true, 1.4F);
		tute.AddQuote("A Wave of Enemies! Creatures spawned from mana!", this, true, 1.4F);
		tute.AddQuote("They're attacking us!",  this, true, 1.4F);
		StartCoroutine(UIManager.instance.Quote(tute.ToArray()));
	}
	...
	IEnumerator Turn3()
	{
		...
		tute.AddQuote("Oh, by the way, mana is very unstable", this, true, 1.4F);
		tute.AddQuote("... and has some side effects.", this, true, 1.4F);
		...
		move.SetMethod( () => {
			...
			tute = new QuoteGroup("Tute");
			tute.AddQuote("Oh, disgusting! Not again!",  this, true, 1.4F);
			tute.AddQuote("And all over my new shoes.",  this, true, 1.4F);
			tute.AddQuote("Collecting enough mana makes you Level Up!",  this, true, 1.4F);
			StartCoroutine(UIManager.instance.Quote(tute.ToArray()));
		});
